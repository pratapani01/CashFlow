...
const lowBalanceBannerText = document.getElementById("lowBalanceBannerText");
...
// still in flight when the user switches currency. Refreshed against
// live USD-based rates (~₹96.3/USD at time of writing) -- these will
// drift over time, which is fine since they're only a fallback.
let exchangeRates = {
    INR: 1,
    USD: 0.0104,
    EUR: 0.0091,
...
function renderSummary() {
    ...
    if (totalSalary > 0 && balance <= totalSalary * 0.10) {
        ...
        const percent = ((balance / totalSalary) * 100).toFixed(1);
        lowBalanceBannerText.innerText =
            `Your remaining balance is ${currentCurrency} ${(balance * rate).toFixed(2)} — only ${percent}% of your total salary. Consider reviewing your expenses.`;
...
/* =========================================================
   Currency Conversion
   ---------------------------------------------------------
   Frankfurter's API moved from api.frankfurter.app to
   api.frankfurter.dev, and its query param is "base" (not
   "from"). It also doesn't accept INR as a base currency at
   all -- only as a target. So we fetch rates with base=USD
   (which INR/EUR/GBP are all valid targets for) and derive
   INR-relative rates ourselves via cross-rate math, since
   every amount in this app is stored in INR.
========================================================= */
...
async function loadRates() {
    ...
    try {
        ...
        const usdToInr = data && data.rates && data.rates.INR;
        const usdToEur = data && data.rates && data.rates.EUR;
        const usdToGbp = data && data.rates && data.rates.GBP;

        if (usdToInr && usdToEur && usdToGbp) {
            exchangeRates = {
                INR: 1,
                USD: 1 / usdToInr,
                EUR: usdToEur / usdToInr,
                GBP: usdToGbp / usdToInr
            };
        } else {
...
async function generateCashFlowPdf() {
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text("Generated by CashFlow Dashboard", margin, footerY);
